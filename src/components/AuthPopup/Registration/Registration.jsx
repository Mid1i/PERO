...
import {registrationInputs as inputs} from "@utils/constants";
...
export default function Registration() {
    ...
    const genderInputsRender = (id, label, onChangeInputs) => {
        return (
            <div className="auth-gender__wrapper">
                <label className="auth-gender__wrapper-label" htmlFor={id}>{label}</label>
                <input
                    autoComplete="none"
                    className="auth-gender__wrapper-input"
                    id={id} 
                    name="gender"
    ...
    return (
        <>
            ...
            <p className="auth-popup__subtitle">
                <span className="auth-popup__subtitle-text">Уже есть аккаунт?</span>
                <span className="auth-popup__subtitle-btn" onClick={onClickChangeStep}>Войдите</span>
            </p>
            ...
            <form autoComplete="off" className="auth-popup__form" onSubmit={onSubmitForm}>
                {inputs.map((item, index) => (
                    <div className="auth-popup__form-wrapper auth-form" key={index}>
                        ...
                        {item.type === 'password' && (
                            <svg className="auth-form__icon" onClick={item.id === 'password' ? setVisiblePassword : setVisiblePasswordCheck} height="18" viewBox="0 0 25 18" width="25">
                ...
                <div className="auth-popup__form-gender auth-gender">
