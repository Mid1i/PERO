...
function SearchBar({ className="search-bar", onCancel=false }) {
    ...
    return (
        <div className={ className } >
            <input className="search-bar__input" type="text" inputMode="search" placeholder="Что Вы ищете?" value={ searchValue } onChange={ (event) => setSearchValue(event.target.value) } onKeyDown={ (event) => onHandleKeyDown(event.key) } />
