...
function SearchBar({ className="search-bar", onCancel=false }) {
    ...
    useEffect(() => {
        if (queryString.parse(search)["search"]) {
            setValue(queryString.parse(search)["search"]);
        }
    }, [search])

    
    function onHandleKeyDown(key) {
        if (key === "Enter") {
    ...
    return (
        <div className={ className } >
            <input className="search-bar__input" type="text" placeholder="Что Вы ищете?" value={ value } onChange={ (event) => setValue(event.target.value) } onKeyDown={ (event) => onHandleKeyDown(event.key) } />
            <div className="search-bar__icons">
                <svg width="20" height="20" fill="none" onClick={ () => navigate(`/catalog/?search=${value}`) }>
