...
export default function SneakerCard({id, preview, name, price, page = 'home'}) {
    const {isInFavorite, onAddToFavorite} = useContext(appContext);
    const navigate = useNavigate();
    
    return (
        <div title={name} className={classNames("goods__content-item goods-item", page === 'catalog' && "goods-item--catalog")}>
            <span className="goods-item__icon" onClick={() => onAddToFavorite(id)}>
                <svg className={classNames(isInFavorite(id) && "liked")} height="15" viewBox="0 0 15 15" width="15">
