...
import {passwordResetInputs as inputs} from "@utils/constants";
import {isEmptyValue} from "@utils/helpers";
import {fetchAuthPasswordReset} from "@api";


import "./AuthPasswordReset.style.scss";
...
export default function AuthPasswordReset() {
    ...
    useEffect(() => {
        if (inputsValue.passwordReset !== inputsValue.passwordCheckReset && inputsValue.passwordCheckReset) {
            setInputsError({...inputsError, 'passwordCheckReset': 'Пароли не совпадают'});
        } else {
            setInputsError({...inputsError, 'passwordCheckReset': ''});
        }
    ...
    const onErrorHandling = (error) => {
        if (error.response.status === 400 || error.response.status === 401) {
            if (typeof error.response.data === 'object') {
                setInputsError({'passwordReset': error.response.data.password});
            } else {
                setInputsError({'exist': error.response.data});
            }
        }
    }
    ...
    const onSubmitForm = (event) => {
        ...
        if (inputsValue.password === inputsValue.passwordCheck) {
            axios.put(fetchAuthPasswordReset(params.uuid), {'password': inputsValue.passwordReset})
                 .then(() => navigate('/'))
    ...
    const onCheckingVisibility = () => {
        if (visiblePassword && !visiblePasswordCheck) {
            return 'text';
    ...
    return (
        <>
            ...
            <div className="content">
                <div className="content__reset">
                    <h1 className="content__reset-title">Сброс пароля</h1>
                    <p className="content__reset-error">{inputsError?.exist || ''}</p>
                    <form autoComplete="off" className="content__reset-form reset-form" onSubmit={onSubmitForm}>
                        {inputs.map((item, index) => (
                            <div className="reset-form__wrapper" key={index}>
                                <input 
                                    {...item}
                                    className="reset-form__wrapper-input"
                                    onChange={(event) => setInputsValue({...inputsValue, [item.id]: event.target.value})}
                                    type={onCheckingVisibility()}
                                    value={isEmptyValue(inputsValue, item.id)}
                                    required
                                />
                                <label 
                                    className={classNames("reset-form__wrapper-label", 
                                        !!inputsError?.[item.id] && "reset-form__wrapper-label--error",
                                        !!inputsValue?.[item.id] && "reset-form__wrapper-label--active"
                                    )} 
                                    htmlFor={item.id}
                                >
                                ...
                                {item.type === 'password' && (
                                    <svg className="reset-form__wrapper-icon" onClick={item.id === 'password' ? setVisiblePassword : setVisiblePasswordCheck} height="18" viewBox="0 0 25 18" width="25">
                        ...
                        <button className="reset-form__btn" type="submit">Сбросить пароль</button>
                    </form>
                </div>
            </div>
            <Footer/>
